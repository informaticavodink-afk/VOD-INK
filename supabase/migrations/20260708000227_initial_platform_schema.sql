-- VOD INK P0 schema: studios, roles, artists, consents, private PDF storage and RLS.
-- Safe to run on Supabase Postgres. Service role bypasses RLS for backend jobs.
...
create type public.consent_status as enum ('draft', 'pending_artist', 'signed', 'upload_error', 'cancelled');
...
set search_path = ''
as $$
  select a.id, a.studio_id, a.full_name, a.qualification, a.photo_url
  from public.artists a
...
-- Data API privileges. RLS still decides which rows each authenticated user can access.
...
-- Initial VOD INK seed. Profiles are linked later when Supabase Auth users are created.
